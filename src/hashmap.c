...
// For specific types of maps, to be defined below...
static unsigned long string_hash_func(char *keystring);
static unsigned long int_hash_func(int *integer);

static int intcmp(int *a, int *b);

int hashmap_init_strmap(hashmap *map) {
    return hashmap_init(map, (hashmap_hash_func) &string_hash_func,
                             (hashmap_keycmp_func) &strcmp);
}

int hashmap_init_intmap(hashmap *map) {
    return hashmap_init(map, (hashmap_hash_func) &int_hash_func,
                             (hashmap_keycmp_func) &intcmp);
}
...
int hashmap_get(hashmap *map, hashmap_key_ptr key, hashmap_value_ptr *result) {
    ...
    int curr = (int) (hash_func(key) % map->table_size);
 ...
 */
unsigned long string_hash_func(char *keystring) {
...
unsigned long int_hash_func(int *keyval) {
    unsigned long key = *keyval;
	/* Robert Jenkins' 32 bit Mix Function */
	key += (key << 12);
	key ^= (key >> 22);
	key += (key << 4);
